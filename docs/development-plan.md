...
- 服药计划支持每天、每隔若干天、每周指定日期和服停循环；每天可配置多个时间点及独立剂量。
...
| `medication_times` | 一天内一个或多个服用时间及可选的独立剂量 |
...
药物周期支持 `daily`、`interval_days`、`weekly` 和 `cycle`。周期和来源均保留类型及版本字段，后续增加每 N 小时或新提醒来源时，不需要重写投递核心。
